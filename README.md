...
2. [install.sh](install.sh) — Bash-скрипт для автоматической установки всех пакетов из `packages.txt`.  
  Основные функции:
  1. Обновление системы (`sudo pacman -Syu`)
  2. Установка официальных пакетов через pacman
  3. Установка yay, если он отсутствует, от обычного пользователя
  4. Установка пакетов из AUR через yay
  5. Проверка уже установленных пакетов и пропуск дубликатов
3. [README.md](README.md) — это руководство по использованию скрипта.

## Подготовка к использованию

1. Убедитесь, что у вас установлена свежая [Arch Linux](https://wiki.archlinux.org/title/Arch_Linux).
...
> Важно: не запускайте скрипт полностью от root. sudo используется внутри скрипта только для официальных пакетов pacman. AUR-пакеты и установка yay выполняются от обычного пользователя.
